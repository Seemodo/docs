...
Inhalt, Struktur und Styling einer Website scrapen zur Verwendung als Referenz für KI-Generierung. Powered by Firecrawl für zuverlässige Extraktion.
...
<ParamField body="formats" type="array" default="['markdown', 'html']">
  ...
  - `screenshot` - Seiten-Screenshot (erfordert Firecrawl Pro)
</ParamField>
...
sequenceDiagram
    ...
    API->>Firecrawl: URL scrapen
    Firecrawl-->>API: Inhalt zurückgeben
...
```json
{
  ...
  "data": {
    ...
    "content": "Dies ist eine Mock-Antwort für https://example.com. Konfiguriere FIRECRAWL_API_KEY um echtes Scraping zu aktivieren.",
    "markdown": "# Beispiel Website\n\nDies ist Mock-Inhalt für Demonstrationszwecke."
  }
}
```
...
```env
FIRECRAWL_API_KEY=dein-firecrawl-api-key
```

API Key erhältst du auf [firecrawl.dev](https://firecrawl.dev).

## Einschränkungen

- Manche Websites blockieren automatisches Scraping
- JavaScript-lastige SPAs rendern möglicherweise nicht vollständig
- Rate-Limits gelten basierend auf deinem Firecrawl-Plan
- Screenshots erfordern Firecrawl Pro
