...
│   ├── OpenCodePanel.tsx # AI chat interface
...
flowchart LR
    ...
    OpenCodeServer -->|Code Changes| Files
...
stateDiagram-v2
    ...
    ViteStarting --> OpenCodeStarting: Vite Ready
    OpenCodeStarting --> Ready: All Services Up
...
sequenceDiagram
    ...
    participant OpenCode as OpenCode AI
    participant Sandbox
    participant Vite

    User->>Frontend: Enter prompt
    Frontend->>API: POST /api/generate-screen
    API->>OpenCode: Send prompt with context
    
    loop Streaming Response
        OpenCode->>API: Code changes SSE
        API->>Frontend: Stream updates
    end
    
    OpenCode->>Sandbox: Write files
...
| OpenCode SDK | AI coding assistant |
