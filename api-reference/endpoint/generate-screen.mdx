...
sequenceDiagram
    participant Client
    participant API
    participant SeemodAI
    participant Sandbox

    Client->>API: POST /generate-screen
    API->>API: Build agent prompt with context
    API->>SeemodAI: Send generation request
    SeemodAI->>Sandbox: list_files()
    SeemodAI->>Sandbox: read_file(App.tsx)
    SeemodAI->>Sandbox: write_file(pages/page.tsx)
    SeemodAI->>Sandbox: Update App.tsx routes
    SeemodAI->>Sandbox: Check Vite logs
    SeemodAI-->>API: Generation complete
