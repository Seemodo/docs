...
```bash
curl -X POST http://localhost:3000/api/edit-screen \
...
<ResponseField name="usedMorph" type="boolean">
  Whether Morph Fast Apply was used for the edit.
</ResponseField>

<ResponseField name="useOpenCode" type="boolean">
  Whether OpenCode AI was used for the edit.
</ResponseField>
...
```mermaid
flowchart TD
    A[Edit Request] --> B{OpenCode Available?}
    B -->|Yes| C[Use OpenCode AI]
    B -->|No| D{Morph API Key Set?}
    C --> E[Write to Sandbox]
    D -->|Yes| F[Generate Instructions]
    D -->|No| G[Full HTML Regeneration]
    F --> H[Apply with Morph]
...
### Strategy 1: OpenCode AI (Preferred)

When a Modal sandbox is active, uses OpenCode to directly edit files:
...
### Strategy 2: Morph Fast Apply

When `MORPH_API_KEY` is configured:

1. AI generates edit instructions and minimal code snippet
2. Morph merges the snippet into existing code
...
- Morph provides faster edits when API key is configured
- OpenCode provides the most reliable edits for complex changes
